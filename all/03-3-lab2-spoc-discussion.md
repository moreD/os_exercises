...
>  
首先建立GDT，设定kernel数据段和代码段，启动段机制
然后载入pmm_manager，通过bootloader时得到的物理内存信息建立并载入页表
最后通过设置CR0寄存器启动分页
...
trapframe at 0x7b6c
  ...
  fs   0x----0023
  gs   0x----0023
  trap 0x00000025 Hardware Interrupt
  err  0x00000000
  eip  0x0010006b
  cs   0x----0008
  flag 0x00000206 PF,IF,IOPL=0
kernel panic at kern/trap/trap.c:192:
