...
首先建立IDT，IDT中的表项以中断号为索引，包括段选择子和偏移量，可以在GDT中定位中断服务例程的地址。在IDT中加入处理时钟中断的对应表项。
通过lidt使CPU加载IDT，对8259中断控制器的初始化
通过outb等初始化时钟，并打开中断
...
>
首先实现bootloader进入保护模式（A20，GDT）
然后实现bootloader加载kernel（硬盘读取、确定入口点）
然后实现kernel（中断处理、系统调用等）
